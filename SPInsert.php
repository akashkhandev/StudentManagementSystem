...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($_POST["remarks"])) {
	...
	$db->insert('studentperformance',array('EnrollmentNumber'=>$enrollnum,'DateMonth'=>$datemonth,'DateYear'=>$dateyear,'ClassName'=>$classname, 'Section'=>$section, 'Att_Per'=>$att_per, 'CA_Per'=>$ca_per, 'Behaviour'=>$behaviour, 'QMR_Per'=>$qmr_per, 'CCA_Per'=>$cca_per, 'Remarks'=>$remarks));
	if ($db->rowsEffected()){
		echo '<script language="javascript">';
		echo 'alert("Successfully Saved"); location.href="SPInsertForm.php"';
        echo '</script>';
	}
	else{
		die("Data not Saved");
